...
void ICACHE_FLASH_ATTR spi(uint8 c) {
	int i;
...
void ICACHE_FLASH_ATTR blackLcd() {
	int res;
	easygpio_outputSet(LCD_SCE, 0);
	easygpio_outputSet(LCD_D_C, 1);
	for(res=0;res<504;res++) {
...
void ICACHE_FLASH_ATTR gotoXY(unsigned char x,unsigned char y) {
...
void ICACHE_FLASH_ATTR showChar(const unsigned char *c,  uint8 charCol, uint8 charRow) {
	const uint8 w = 5;
	uint8 pxCol;

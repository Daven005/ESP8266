...
void ICACHE_FLASH_ATTR publishData(MQTT_Client* client) {
	if (mqttConnected) {
		...
		os_sprintf(data, (const char*) "{ \"Type\":\"Level\", \"Value\":%d}", latestValue);
		MQTT_Publish(client, topic, data, strlen(data), 0, false);
		TESTP("%s=>%s\n", topic, data);
		os_sprintf(topic, (const char*) "/Raw/%s/1/info", sysCfg.device_id);
		os_sprintf(data, (const char*) "{ \"Type\":\"Level\", \"Value\":%d}", periodValue);
		MQTT_Publish(client, topic, data, strlen(data), 0, false);
		TESTP("%s=>%s\n", topic, data);
		os_sprintf(topic, (const char*) "/Raw/%s/2/info", sysCfg.device_id);
		os_sprintf(data, (const char*) "{ \"Type\":\"Level\", \"Value\":%ld}", cumulativeValue);
		MQTT_Publish(client, topic, data, strlen(data), 0, false);
		TESTP("%s=>%s\n", topic, data);
		periodValue = 0;
		checkMinHeap();
...
void ICACHE_FLASH_ATTR processCb() {
	if (lt2400_ready()) {
		latestValue = lt2400_read();
		if (latestValue != 0xffffffff) {
			periodValue += latestValue;
			cumulativeValue += latestValue;
		}
	}
}

void ICACHE_FLASH_ATTR printAll() {
	os_printf("Test %01x %d %ld %ld\n", lt2400_status(), latestValue, periodValue, cumulativeValue);
}

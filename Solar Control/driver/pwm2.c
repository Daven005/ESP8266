...
//IMPORTANT:   t must be u32 type.  u16, 450~800, err!!!!!!!!!!!
...
static void ICACHE_FLASH_ATTR pwm_insert_sort(struct pwm_single_param pwm[], uint8 n) {
	uint8 i;
...
//os_printf("init_set:%u\n",US_TO_RTC_TIMER_TICKS(pwm.period)-17);
		//run_pwm_toggle = 0;
		for (i = 0; i < local_channel; i++) {
			os_memcpy(&run_pwm_single[run_pwm_toggle][i], &local_single[i],
					sizeof(struct pwm_single_param));
		}
	...
	} else {       // update to another run_pwm_single
 ...
 * Returns      : NONE
 *******************************************************************************/
void ICACHE_FLASH_ATTR pwm_set_duty(uint32 duty, uint8 channel) {
	if (duty < 1) {
		pwm.duty[channel] = 0;
	} else {
		pwm.duty[channel] = duty;
	}
	TESTP("channel:%u, duty:%u\n", channel, pwm.duty[channel]);
}
...
// hdl gpio , read_reg cost  0.4 us
	if (pwm_single[pwm_current_channel].h_time > 6) {
		...
		if (pwm_single[pwm_current_channel].h_time < 28) {
			u16 b = 0;
			u16 tmp = (pwm_single[pwm_current_channel].h_time - 7);
			if (tmp & 0x1)
				b = 5 * ((tmp + 1) >> 1) - 3;
			else
				b = 5 * ((tmp + 2) >> 1) - 5;
 ...
 *******************************************************************************/
#include "gpio.h"
void ICACHE_FLASH_ATTR pwm_init(uint32 period, uint32 *duty, uint32 pwm_channel_num, uint32 (*pin_info_list)[3]) {
	uint8 i;
